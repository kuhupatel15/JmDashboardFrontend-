...
const CourtKachheri = () => {
  ...
  useEffect(() => {
    const fetchCourtKachheriNews = async () => {
      try {
        const response = await fetch(`${BASE_URL}/api/news?category=कोर्ट कचेरी`); 
        const data = await response.json();
        setNewsData(data);
      } catch (error) {
        console.error("Error fetching कोर्ट कचेरी news:", error);
      }
    };
    

    fetchCourtKachheriNews();
  }, []);

  return (
    <div className="container mx-auto py-5 px-10">
      <h1 className="text-5xl font-bold text-center mb-5">कोर्ट कचेरी खबरें</h1>
